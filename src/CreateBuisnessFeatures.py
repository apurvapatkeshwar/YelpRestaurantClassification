...
def process_data_set(photo_to_biz_fl, labels_fl, image_feature_fl, business_feature_fl, comb_mode):
    ...
    with h5py.File(image_feature_fl, 'r') as f:
        X = pd.DataFrame(np.zeros((len(labels), f['feature'].shape[1]),
                        dtype=f['feature'].dtype), index=labels.index)
...
if __name__ == '__main__':
    for (model_weights, _, _, image_feature_fl, business_feature_fl) in cfg.models:
        for comb_mode in cfg.comb_modes:
            for (mode, _, photo_to_biz_fl, labels_fl) in cfg.data_sets:
